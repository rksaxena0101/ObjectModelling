...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        return questionMap.entrySet().stream().map(ques -> ques.getValue()).collect(Collectors.toList());
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        return questionMap.entrySet().stream().map(val -> val.getValue())
            .filter(quesLevel -> quesLevel.getLevel().equals(level)).collect(Collectors.toList());
    }
