...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
        return contestMap.entrySet().stream().map(contest -> contest.getValue()).collect(Collectors.toList());
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        return contestMap.entrySet().stream().map(val -> val.getValue()).filter(contestLevel -> contestLevel.getLevel().equals(level)).collect(Collectors.toList());
    }
