...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder) {
        return ((scoreOrder.toString().compareTo("ASC") == 0)) ? userRepository.findAll().stream().sorted(Comparator.comparingInt(User::getScore)).collect(Collectors.toList()) 
            : userRepository.findAll().stream().sorted(Comparator.comparingInt(User::getScore).reversed()).collect(Collectors.toList());
    }
